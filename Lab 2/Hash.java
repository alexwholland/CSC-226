...
public class Hash{
	...
	*/
	public int hash(String s){
		...
		for (int i = 0; i < s.length(); i++){
			// Convert the string into a hash of the string. Formula derived from Lab 3 slides.
			asciiCode +=  ((int) s.charAt(i)*Math.pow(7, (s.length()-i-1)) % TableSize);
		}
		return asciiCode % TableSize;
	}
	...
	*/
	public int insert(String s){
		...
		if (T.getElement(hashIndex) == null){
			T.setElement(hashIndex, s);
		}
		else{
			// Use linear probing to find an empty slot in the hash table.
			for (int i = hashIndex + 1; i != hashIndex; i = ((i + 1) % TableSize)){
	...
	*/
	public int find(String s){
		...
		int hashIndex = hash(s);
		if (s.equals(T.getElement(hashIndex))){
			return hashIndex;
		}
		else{
			//Search for the string s in the hash table.
			for (int i = hashIndex + 1; i != hashIndex && T.getElement(i) != null; i = ((i + 1) % TableSize)){
